...
contract NexusTokenTest is Test {
    ...
    function testMint() public {
        ...
        uint256 expectedSupply = 1000 * 10 ** nexusToken.decimals() + mintAmount;
    ...
    function testBurn() public {
        ...
        uint256 initialBalance = nexusToken.balanceOf(owner);

        nexusToken.burn(burnAmount);

        uint256 expectedBalance = initialBalance - burnAmount;
        assertEq(nexusToken.balanceOf(owner), expectedBalance);

        uint256 expectedSupply = 1000 * 10 ** nexusToken.decimals() - burnAmount;
        assertEq(nexusToken.totalSupply(), expectedSupply);
    }
